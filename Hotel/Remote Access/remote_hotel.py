...
支持 hsmdtv 源：读取 hsmd_address_list.txt，将测速选出的 hsmdtv 源 IP:端口
替换到地址列表中的 URL，生成最终输出文件。
"""
...
CHANNEL_CATEGORIES = {
 ...
 "湖南频道": [
     ...
     "邵阳县","隆回电视台","武冈综合","新邵新闻综合","城步电视","洞口综合频道","新宁新闻综合","邵东电视台","凤凰时政","汨罗综合","绥宁电视台","衡南电视台"
 ],
 "湖北频道": [
 ...
 "山西频道": [
      "山西影视","山西文体生活","山西经济与科技","山西黄河","山西社会与法治","太原-1","吕梁-1","吕梁-2","中阳电视台","离石综合"
 ],
 "广东频道": [
      "广东影视", "广东少儿", "广东民生", "广东新闻", "广东经济科教", "广东体育","广东珠江","广东岭南戏曲","广东现代教育", "广州台","广州新闻", "广州影视","广州经济","嘉佳卡通",
      "深圳电视剧频道","深圳都市频道", "深圳少儿","珠海",  
...
CHANNEL_MAPPING = {
        ...
        "兵器科技": ["兵器科技", "CCTV兵器科技", "兵器科技频道","兵器科技HD","国防军事","CCTV兵器"],
        ...
        "央视台球": ["央视台球", "CCTV央视台球","CCTV-央视台球","央视台球HD","CCTV央视台球高清","CCTV台球"],
        "高尔夫网球": ["高尔夫网球", "央视高网", "CCTV高尔夫网球", "高尔夫","高尔夫·网球HD","CCTV高尔夫网球高清"],
        "央视文化精品": ["央视文化精品", "CCTV央视文化精品","央视精品","CCTV精品","CCTV文化"],
        "老故事": ["CCTV老故事"],
        "书画频道": ["书画","书法频道", "书法书画","书法"],
        "卫生健康": ["卫生健康", "CCTV卫生健康"],
        ...
        "CHC影迷电影": ["高清电影","CHC 高清电影","高清影院","CHC高清电影","中国电影","CHC电影","电影"],
    ...
    "北京财经": ["BTV财经"],
        ...
        "广东影视": ["广东影视-1","测试频道22","广东影视高清","南方四"],
        "广东少儿": ["南方五","广东少儿高清",],
        "广东民生": ["广东民生","广东公共",],
        "广东新闻": ["广东新闻","广东新闻高清","广州综合"],
        "广东经济科教": ["广东科技","广东经济科教高清","南方一"],
...
def parse_hsmd_address_list(filepath):
    """
    解析 hsmd_address_list.txt 文件
    支持两种格式:
      格式A (逗号分隔):  频道名,http://ip:port/path
      格式B (空格分隔):  频道名 http://ip:port/path
    返回: list of dict: [{'name': 'CCTV1', 'url': 'http://...', 'line': '原始行'}, ...]
    """
    entries = []
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        for raw in f:
            ...
            entries.append({'name': name, 'url': url, 'raw': raw})
    return entries


def replace_host_in_url(url, new_host):
    """
    将 URL 中的 host:port 替换为新的 host:port
    例如: http://old-ip:port/path -> http://new-host:port/path
    """
    parsed = urlparse(url)
    # new_host 可能带端口也可能不带
...
def process_hsmdtv_channels(host, source_index):
    ...
    try:
        ...
        for item in parsed_entries:
            ...
            # 替换 host 为测速选出的 hsmdtv 源
            new_url = replace_host_in_url(original_url, host)
            ...
            entries.append({
                ...
                'index': source_index,
                'speed': 0  # hsmdtv 源的速度会在外层赋值
            })
            log(f"   ✅ {name}: {original_url} -> {new_url}")
...
def save_hsmd_updated_list(host, output_path=None):
    """
    ...
    try:
        ...
        for item in parsed_entries:
            ...
            lines.append(f"{item['name']},{new_url}")
        content = "\n".join(lines) + "\n"
...
def fetch_remote_sources():
    ...
    # 抓取频道
    all_entries = []
    hsmdtv_host = None  # 记录选中的 hsmdtv 源 host
    ...
    for idx, source in enumerate(final_sources):
        ...
        if match_type == 'hsmdtv':
            hsmdtv_host = source['host']
            # hsmdtv 需要从 hsmd_address_list.txt 生成频道
            hsmd_entries = process_hsmdtv_channels(source['host'], idx)
            # 赋值速度信息
            for e in hsmd_entries:
                e['speed'] = source['speed']
            all_entries.extend(hsmd_entries)
    ...
    # ========== 额外保存 hsmd 更新后的地址列表 ==========
    if hsmdtv_host:
        updated_list_path = os.path.join(
            os.path.dirname(OUTPUT_TXT) or '.',
            "hsmd_address_list.updated.txt"
        )
        save_hsmd_updated_list(hsmdtv_host, updated_list_path)

    del results_with_speed, valid_results, final_sources, all_entries
    gc.collect()
